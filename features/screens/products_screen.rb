class ProductScreen
  def initialize
    @wishlist_button = Elements.new(:id, "wish_list")
  ...
  def open_chosen_category(category)
    @chosen_category = Elements.new(:xpath, "//*[@resource-id=\"com.view9.foreveryng:id/sub_cat_child\"]/*[child::*[@resource-id=\"com.view9.foreveryng:id/listTitle\"][contains(@text, \"#{category}\")]]")

    @chosen_category.click
  end

  def add_first_item_to_wishlist
    all_product_name = @product_names.get_multiple_elements
    @wishlist_product_name = all_product_name.first.text
    all_wishlist_buttons = @wishlist_button.get_multiple_elements
  ...
  def verify_added_item
    expected = @wishlist_product_name
